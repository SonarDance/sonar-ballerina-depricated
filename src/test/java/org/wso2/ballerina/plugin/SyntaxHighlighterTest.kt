...
class SyntaxHighlighterTest : AbstractSensorTest() {
    ...
    @Test
    fun `file with class`() {
        val fileToTest = Path.of("src", "test", "resources", "highlighting", "fileWithClass.bal")

        val inputFile: InputFile = createInputFile("file1.bal", fileToTest.readText())

        context.fileSystem().add(inputFile)
        BallerinaSensor(checkFactory(), fileLinesContextFactory, DefaultNoSonarFilter(), language()).execute(context)
