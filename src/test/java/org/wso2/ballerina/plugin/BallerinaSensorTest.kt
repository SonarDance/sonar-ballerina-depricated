...
import org.wso2.ballerina.testing.assertTextRange
import org.wso2.ballerina.plugin.BallerinaCheckList.BALLERINA_CHECKS
import java.io.IOException
import kotlin.time.ExperimentalTime

@ExperimentalTime
internal class BallerinaSensorTest : AbstractSensorTest() {
...
// Following Tests have been commented out as it's related to the android or desktop environment which are not implemented in Ballerina
//    @Test
...
//        context.fileSystem().add(inputFile)
//
//        mockkStatic("org.wso2.ballerina.plugin.BallerinaSensor")
//        every { environment(any()) } returns Environment(listOf("file1.bal"), LanguageVersion.LATEST_STABLE)
//
//        val checkFactory = checkFactory("S1764")
...
//    }
    ...
    fun test_fail_parsing() {
        val inputFile = createInputFile("file1.bal", "enum class A { <!REDECLARATION!>FOO<!>,<!REDECLARATION!>FOO<!> }")
        context.fileSystem().add(inputFile)
        val checkFactory = checkFactory("S1764")
        sensor(checkFactory).execute(context)
        ...
        Assertions.assertThat(analysisError.message()).isEqualTo("Unable to parse file: file1.bal")
    ...
    fun test_fail_reading() {
        val inputFile = spyk(createInputFile("file1.bal", "class A { fun f() = TODO() }"))
        context.fileSystem().add(inputFile)
        every { inputFile.contents() } throws IOException("Can't read")
        every { inputFile.toString() } returns "file1.bal"
        ...
        Assertions.assertThat(logTester.logs(LoggerLevel.ERROR)).contains("Cannot read 'file1.bal': Can't read")
    }

    @Test
    fun test_with_classpath() {
        val settings = MapSettings()
        settings.setProperty(BallerinaPlugin.SONAR_JAVA_BINARIES, "classes/")
        context.setSettings(settings)
        val inputFile = createInputFile("file1.bal", "class A { fun f() = TODO() }")
        context.fileSystem().add(inputFile)
        val checkFactory = checkFactory("S1764")
        sensor(checkFactory).execute(context)
        val analysisErrors = context.allAnalysisErrors()
    ...
    fun test_with_blank_classpath() {
        val settings = MapSettings()
        settings.setProperty(BallerinaPlugin.SONAR_JAVA_BINARIES, " ")
        context.setSettings(settings)
        val inputFile = createInputFile("file1.bal", "class A { fun f() = TODO() }")
        context.fileSystem().add(inputFile)
        val checkFactory = checkFactory("S1764")
    ...
    private fun failFastSensorWithEnvironmentSetup(failFast: Boolean?): BallerinaSensor {
        mockkStatic("org.wso2.ballerina.plugin.BallerinaCheckList")
        every { BALLERINA_CHECKS } returns listOf(ExceptionThrowingCheck::class.java)

        context.apply {
            setSettings(MapSettings().apply {
                failFast?.let { setProperty(BallerinaPlugin.FAIL_FAST_PROPERTY_NAME, it) }
            })

            fileSystem().add(createInputFile("file1.bal", "class A { fun f() = TODO() }"))
        }
    ...
    fun `ensure failFast is working when set`() {
        ...
        assertThrows<IllegalStateException>("Exception in 'KtChecksVisitor' while analyzing 'file1.bal'") {
        ...
        assertThat(logTester.logs(LoggerLevel.ERROR))
            .containsExactly("Cannot analyse 'file1.bal' with 'KtChecksVisitor': This is a test message")
    }
...
//                setProperty(BallerinaPlugin.BALLERINA_LANGUAGE_VERSION, "1.3")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        val sensorContext = mockk<SensorContext> {
//            every { config() } returns ConfigurationBridge(MapSettings().apply {
//                setProperty(BallerinaPlugin.BALLERINA_LANGUAGE_VERSION, "foo")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        val sensorContext = mockk<SensorContext> {
//            every { config() } returns ConfigurationBridge(MapSettings().apply {
//                setProperty(BallerinaPlugin.BALLERINA_LANGUAGE_VERSION, "  ")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        val sensorContext = mockk<SensorContext> {
//            every { config() } returns ConfigurationBridge(MapSettings().apply {
//                setProperty(BallerinaPlugin.COMPILER_THREAD_COUNT_PROPERTY, "42")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        val sensorContext = mockk<SensorContext> {
//            every { config() } returns ConfigurationBridge(MapSettings().apply {
//                setProperty(BallerinaPlugin.COMPILER_THREAD_COUNT_PROPERTY, "0")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        assertThat(environment.configuration.get(CommonConfigurationKeys.PARALLEL_BACKEND_THREADS)).isNull()
//        assertThat(logTester.logs(LoggerLevel.WARN))
//            .containsExactly("Invalid amount of threads specified for ${BallerinaPlugin.COMPILER_THREAD_COUNT_PROPERTY}: '0'.")
//        assertThat(logTester.logs(LoggerLevel.DEBUG))
//            .contains("Using the default amount of threads")
//    }
//
...
//        val sensorContext = mockk<SensorContext> {
//            every { config() } returns ConfigurationBridge(MapSettings().apply {
//                setProperty(BallerinaPlugin.COMPILER_THREAD_COUNT_PROPERTY, "foo")
//            })
//        }
//
//        val environment = environment(sensorContext)
...
//        assertThat(environment.configuration.get(CommonConfigurationKeys.PARALLEL_BACKEND_THREADS)).isNull()
//        assertThat(logTester.logs(LoggerLevel.WARN))
//            .containsExactly("${BallerinaPlugin.COMPILER_THREAD_COUNT_PROPERTY} needs to be set to an integer value. Could not interpret 'foo' as integer.")
//        assertThat(logTester.logs(LoggerLevel.DEBUG))
//            .contains("Using the default amount of threads")
//    }
    ...
    private fun assertAnalysisIsIncremental() {
        ...
        assertThat(logTester.logs(LoggerLevel.DEBUG))
            .contains("The Ballerina analyzer is running in a context where it can skip unchanged files.")
        assertThat(logTester.logs(LoggerLevel.INFO))
            .contains("Only analyzing 2 changed ballerina files out of 3.")
    }
    ...
    private fun assertAnalysisIsNotIncremental() {
        ...
        assertThat(logTester.logs())
            .doesNotContain("The Ballerina analyzer is working in a context where it can skip unchanged files.")
    }
    ...
    private fun sensor(checkFactory: CheckFactory): BallerinaSensor {
        return BallerinaSensor(checkFactory, fileLinesContextFactory, DefaultNoSonarFilter(), language())
    }
}
